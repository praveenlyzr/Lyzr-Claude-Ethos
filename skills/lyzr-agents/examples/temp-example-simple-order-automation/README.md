...
|---|---|---|
| `GET /inventory/{sku}` | `?want=N` *(optional)* | `{sku, quantity, found}` — and with `?want`: `{want, sufficient:"yes"\|"no", shortfall, restock_qty}` |
...
# out of stock → restock the suggested amount, then order (delayed 5 days)
